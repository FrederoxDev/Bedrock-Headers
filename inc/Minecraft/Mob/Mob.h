...
class CompassSpriteCalculator {
 ...
 public:
  ...
  int update(Actor &, bool);
  int updateFromPosition(const BlockSource &, float, float, float, bool, bool);
  static int calculateFrame(const BlockSource &, const Vec3 &, float);
  static int calculateFrame(const BlockSource &, float, float, float);
};
...
class ClockSpriteCalculator {
 ...
 public:
  ...
  int getFrame(void) const;
  int update(Actor &, bool);
  int update(const BlockSource &, const Vec3 &, bool);
  static int calculateFrame(const Actor &);
  static int calculateFrame(const BlockSource &, const Vec3 &);
};
...
class MovementInterpolator {
 ...
 public:
  ...
  int getPositionSteps(void) const;
  int getRotationSteps(void) const;
...
class Mob : public Actor {
 public:
  enum class TravelType : int { Water, Lava, Ground, Air };
 ...
 protected:
  ...
  bool mDead;
  char filler0[16]; //Weak<VillageLegacy> mVillageLegacy;
 ...
 public:
  ...
  virtual const TextureUVCoordinateSet *getItemInHandIcon(const ItemStack &,
                                                          int);
  ...
  virtual const std::vector<ItemStack, std::allocator<ItemStack> > *
  getAllArmor() const;
  virtual std::vector<int, std::allocator<int> > getAllArmorID() const;
  virtual const std::vector<ItemStack, std::allocator<ItemStack> > *getAllHand()
      const;
  virtual const std::vector<ItemStack, std::allocator<ItemStack> > *
  getAllEquipment() const;
 ...
 protected:
  virtual std::unique_ptr<BodyControl, std::default_delete<BodyControl> >
  initBodyControl();
